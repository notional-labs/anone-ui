...
const ProposalList = ({ accounts }) => {
  ...
  const wrapSetSelect = useCallback(
    (val) => {
  ...
  const wrapSetShowCreateProposal = useCallback(
    (val) => {
  ...
  const wrapSetShowDeposit = useCallback(
    (val) => {
      setShowDeposit(val);
    },
    [setShowDeposit]
  ...
  const handleEnter = (e) => {
    e.target.style.backgroundImage =
      "Linear-Gradient(263.6deg, #4D4D4D 0%, #000000 100%)";
    e.target.style.border = "solid 2px #5dfc8a";
  };

  const handleLeave = (e) => {
    e.target.style.backgroundImage =
      "Linear-Gradient(#2eff66 0%, #5956fc 100%)";
  };
    ...
    <div style={style.container}>
        ...
        <Modal show={show} onHide={handleClose} backdrop="static">
          ...
          <Modal.Body style={{ backgroundColor: "#4D4D4D" }}>
            <VoteModal
              ...
              id={
                proposals[selectProposal] &&
                proposals[selectProposal].proposal_id
              }
              wrapSetShow={wrapSetShow}
            />
          </Modal.Body>
      ...
      <>
        <Modal
          ...
          onHide={handleCloseCreateProposal}
          backdrop="static"
        >
          <Modal.Header
            style={{
